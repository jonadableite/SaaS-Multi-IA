    ...
    return {
      auth: {
        ...
        >(
          ...
          // Ensure user image from database is used
          if (!organization) {
            return {
              ...session,
              user: {
                ...session.user,
                ...user,
                image: user.image || session.user.image, // Prefer database image, fallback to session image
              },
          ...
          // Ensure user image from database is used (Better Auth session.user may not have updated image)
          return {
            ...session,
            user: {
              ...session.user,
