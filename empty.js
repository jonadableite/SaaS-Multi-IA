...
const createEmptyModule = (moduleName = 'unknown') => {
  ...
  const handler = {
    ...
    ownKeys: (target) => {
      // Return all keys including diagnostic properties
      const keys = ['default', '__esModule']
      // Add diagnostic properties if they exist
      if (target._getContaminationReport) {
        keys.push('_getContaminationReport')
      }
      if (target._resetTracking) {
        keys.push('_resetTracking')
      }
      return keys
    },
    ...
    getOwnPropertyDescriptor: (target, prop) => {
      ...
      // Provide descriptors for diagnostic properties
      if (prop === '_getContaminationReport' && target._getContaminationReport) {
        return {
          value: target._getContaminationReport,
          writable: false,
          enumerable: false,
          configurable: false,
        }
      }
      if (prop === '_resetTracking' && target._resetTracking) {
        return {
          value: target._resetTracking,
          writable: false,
          enumerable: false,
          configurable: false,
        }
...
if (DEV_MODE && typeof globalThis !== 'undefined') {
  ...
  // Check if property already exists to avoid redefinition error
  if (!globalThis.hasOwnProperty('__IGNITER_BUNDLE_DIAGNOSTICS__')) {
    Object.defineProperty(globalThis, '__IGNITER_BUNDLE_DIAGNOSTICS__', {
      ...
      enumerable: false,
      configurable: true, // Make configurable to allow updates if needed
      writable: false,
    })
  }
